  ...
  */
  ...
  triangulate(std::vector<VertexType>& vertices) {
    ...
    for (auto p = begin(vertices); p != end(vertices); p++) {
      ...
      polygon.erase(std::remove_if(begin(polygon),
                                   end(polygon),
    ...
    for (const auto t : _triangles) {
      if (std::find(_edges.begin(), _edges.end(), t.e1) == _edges.end()) {
        _edges.push_back(t.e1);
      }
      if (std::find(_edges.begin(), _edges.end(), t.e2) == _edges.end()) {
        _edges.push_back(t.e2);
      }
      if (std::find(_edges.begin(), _edges.end(), t.e3) == _edges.end()) {
